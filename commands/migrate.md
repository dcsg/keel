...
Migration: {.dof/ or .conductor/} → keel
  ...
  Tool adapters to update:
    CLAUDE.md                           — regenerated for keel
    .cursorrules                        — {regenerated or removed}
    .github/copilot-instructions.md     — {regenerated or removed}
...
#### 4.2 — Migrate soul
Copy `{legacy}/soul.md` to `docs/soul.md`. Clean up any legacy references (remove "dof" or "conductor" mentions, update paths).
...
#### 4.9 — Generate new files
Run `/keel:init` logic for generating:
- `.claude/rules/*.md` — based on detected stack
- `.claude/CLAUDE.md` — generated context loader
- `.claude/settings.json` — hooks

#### 4.10 — Clean up old tool adapters
If `.cursorrules`, `.windsurfrules`, or `.github/copilot-instructions.md` exist and reference dof/conductor, note them for removal (keel is Claude Code only).
